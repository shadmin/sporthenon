insert into  "~Config" values ('robot_pattern', 'googlebot|gigabot|exabot|slurp|baiduspider|bingbot|ahrefsbot', NULL);
...
begin
	...
	-- Cities
	IF (_entity = 'CT' OR _entity = '') THEN
		_query = 'SELECT CT.id, CT.label' || _lang || ', CN.code, string_agg(CAST (EL.id AS VARCHAR), '',''), CT2.label, CT.ref';
		_query = _query || ' FROM "City" CT';
		_query = _query || ' LEFT JOIN "Country" CN ON CT.id_country = CN.id';
		...
		ELSIF (_pattern IS NOT NULL AND _pattern <> '') THEN
			_query = _query || ' AND lower(CT.label' || _lang || ') like ''' || lower(_pattern) || '%''';
		END IF;
		_query = _query || ' GROUP BY CT.id, CT.label' || _lang || ', CN.code, CT2.label, CT.ref';
		_query = _query || ' ORDER BY CT.label' || _lang || ' LIMIT ' || _count;
		OPEN _c FOR EXECUTE _query;
		LOOP
...
$BODY$
  ...
  ALTER TABLE "~RefItem"
   ALTER COLUMN label_rel1 TYPE character varying(200);

   ALTER TABLE "~RefItem"
   ALTER COLUMN label_rel2 TYPE character varying(200);

   ALTER TABLE "~RefItem"
   ALTER COLUMN label_rel3 TYPE character varying(200);
...
$BODY$
  ...
  ALTER TABLE "City"
   ...
   ALTER TABLE "City"
   ALTER COLUMN label_fr TYPE character varying(30);
