ALTER TABLE "~RefItem" ALTER COLUMN label_rel4 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel5 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel6 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel7 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel8 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel9 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel10 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel11 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel12 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel13 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel14 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel15 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel16 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel17 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel18 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel19 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel20 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel21 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel22 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel23 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel24 TYPE character varying(200);
ALTER TABLE "~RefItem" ALTER COLUMN label_rel25 TYPE character varying(200);
...
begin
	...
	IF (_entity ~ 'OL|CN' AND (_entity_ref = 'OR' OR _entity_ref = '')) THEN
		_query = 'SELECT OR_.id, OL.id, YR.id, YR.label, CT.id, CT.label' || _lang || ', CN.id, CN.label' || _lang || ', CT.label, CN.label, OR_.count_gold || '','' || OR_.count_silver || '','' || OR_.count_bronze, OR_.last_update FROM "OlympicRanking" OR_';
	...
	IF (_entity ~ 'YR|PR' AND (_entity_ref = 'HF' OR _entity_ref = '')) THEN
		_query = 'SELECT HF.id, YR.id, YR.label, PR.id, PR.last_name, PR.first_name, LG.id, LG.label, HF.position, HF.last_update FROM "HallOfFame" HF';
	...
	IF (_entity ~ 'PR' AND (_entity_ref = 'RN' OR _entity_ref = '')) THEN
		_query = 'SELECT RN.id, TM.id, TM.label, PR.id, PR.last_name, PR.first_name, LG.id, LG.label, RN.number, RN.last_update FROM "RetiredNumber" RN';
		...
		OPEN _c FOR EXECUTE _query;
		LOOP
			FETCH _c INTO _item.id_item, _item.id_rel1, _item.label_rel1, _item.id_rel2, _item.label_rel2, _item.label_rel3, _item.id_rel3, _item.comment, _item.id_rel4, _item.date3;
			EXIT WHEN NOT FOUND;
			_item.id = _index;
			_item.entity = 'RN';
			RETURN NEXT _item;
	...
	IF (_entity ~ 'CX' AND (_entity_ref = 'TS' OR _entity_ref = '')) THEN
		_query = 'SELECT TS.id, TM.id, TM.label, CX.id, CX.label, CT.id, CT.label' || _lang || ', ST.id, ST.label' || _lang || ', CN.id, CN.label' || _lang || ', CX.label, CT.label, ST.label, CN.label, LG.id, LG.label, TS.date1, TS.date2, TS.last_update FROM "TeamStadium" TS';
...
begin
	...
	LOOP
		...
		END IF;
		IF _type1 <= 10 THEN
			SELECT id_rank1, id_rank2, id_rank3, PR1.last_name || (CASE WHEN length(PR1.first_name) > 0 THEN ', ' || PR1.first_name ELSE '' END), (CASE WHEN length(PR1.first_name) > 0 THEN PR1.first_name || ' ' ELSE '' END) || PR1.last_name, CN1.id, (CASE WHEN _lang = '_fr' THEN CN1.label_fr ELSE CN1.label END), TM1.label, PR2.last_name || (CASE WHEN length(PR2.first_name) > 0 THEN ', ' || PR2.first_name ELSE '' END), (CASE WHEN length(PR2.first_name) > 0 THEN PR2.first_name || ' ' ELSE '' END) || PR2.last_name, CN2.id, (CASE WHEN _lang = '_fr' THEN CN2.label_fr ELSE CN2.label END), TM2.label, PR3.last_name || (CASE WHEN length(PR3.first_name) > 0 THEN ', ' || PR3.first_name ELSE '' END), (CASE WHEN length(PR3.first_name) > 0 THEN PR3.first_name || ' ' ELSE '' END) || PR3.last_name, CN3.id, (CASE WHEN _lang = '_fr' THEN CN3.label_fr ELSE CN3.label END), TM3.label, RD.result1, RD.result2, RD.comment, RD.exa
			INTO _item.id_rel6, _item.id_rel7, _item.id_rel8, _item.label_rel6, _item.label_rel20, _item.id_rel12, _cn1, _tm1, _item.label_rel7, _item.label_rel21, _item.id_rel13, _cn2, _tm2, _item.label_rel8, _item.label_rel22, _item.id_rel14, _cn3, _tm3, _item.txt1, _item.txt2, _item.txt3, _item.txt4
			FROM "Round" RD LEFT JOIN "Athlete" PR1 ON RD.id_rank1 = PR1.id LEFT JOIN "Athlete" PR2 ON RD.id_rank2 = PR2.id LEFT JOIN "Athlete" PR3 ON RD.id_rank3 = PR3.id LEFT JOIN "Country" CN1 ON PR1.id_country = CN1.id LEFT JOIN "Country" CN2 ON PR2.id_country = CN2.id LEFT JOIN "Country" CN3 ON PR3.id_country = CN3.id LEFT JOIN "Team" TM1 ON PR1.id_team = TM1.id LEFT JOIN "Team" TM2 ON PR2.id_team = TM2.id LEFT JOIN "Team" TM3 ON PR3.id_team = TM3.id
			WHERE RD.id = _item.id_item;
			IF _cn1 IS NOT NULL THEN _item.label_rel6 = _item.label_rel6 || '|' || _cn1;
			ELSIF _tm1 IS NOT NULL THEN _item.label_rel6 = _item.label_rel6 || '|' || _tm1; END IF;
...
begin
	...
	'SELECT DISTINCT SP.id, SP.label' || _lang || ' COLLATE "fr_FR", SP.label, CP.id, CP.label' || _lang || ' COLLATE "fr_FR", CP.label, EV.id, EV.label' || _lang || ' COLLATE "fr_FR", EV.label, SE.id, SE.label' || _lang || ' COLLATE "fr_FR", SE.label, SE2.id, SE2.label' || _lang || ' COLLATE "fr_FR", SE2.label, II.id_championship, II.id_event, II.id_subevent, II.id_subevent2, CP.index, EV.index, SE.index, SE2.index, (CASE WHEN II.id_event IS NOT NULL AND II.id_subevent IS NULL AND II.id_subevent2 IS NULL THEN 1 ELSE 0 END) AS o_ii_ev, (CASE WHEN II.id_subevent IS NOT NULL AND II.id_subevent2 IS NULL THEN 1 ELSE 0 END) AS o_ii_se, (CASE WHEN II.id_subevent2 IS NOT NULL THEN 1 ELSE 0 END) AS o_ii_se2
	    ...
	    ' || _filter || ' ORDER BY SP.label' || _lang || ' COLLATE "fr_FR", CP.index, CP.label' || _lang || ' COLLATE "fr_FR", o_ii_ev, EV.index, EV.label' || _lang || ' COLLATE "fr_FR", o_ii_se, SE.index, SE.label' || _lang || ' COLLATE "fr_FR", o_ii_se2, SE2.index, SE2.label' || _lang || ' COLLATE "fr_FR"';
	LOOP
...
$BODY$
 ...
 SELECT ( SELECT count(*) AS count
    ...
    ( SELECT count(*) AS count
           FROM "Team") AS count_team,( SELECT count(*) AS count
           FROM "~Picture") AS count_picture;
