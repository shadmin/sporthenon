...
$BODY$
  ...
  update "Event" set label= upper(substring(label, 1, 1)) || lower(substring(label, 2)) where label not like '%AFL%' and label not like '%NFL%' and label not like '%MVP%' and  label not like '%Grand Prix%'
 and  label not like 'Rally%' and  label not like '%TD%' and  label not like '%Basho%' and  label not like '%PGA%' and  label not like '%RS:X%' and  label not like '%NBA%' and  label not like '%MLB%'
and  label not like '%MotoGP%' and  label not like 'MX%';
...
update "Result" set result1=replace(result1,'/','-') where result1 like '%/%/%';
ALTER TABLE "Result" ENABLE TRIGGER "TriggerRS";

ALTER TABLE "Round" DISABLE TRIGGER "TriggerRD";
update "Round" set result1=replace(result1,'/','-') where result1 like '%/%/%';
ALTER TABLE "Round" ENABLE TRIGGER "TriggerRD";
...
$BODY$
  ...
  insert into  "~Config" values ('USL_STATS_EVENT_LABEL', 'Statistical leaders');
