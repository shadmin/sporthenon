...
public class HtmlConverter {
	...
	public static StringBuffer getHeader(HttpServletRequest request, short type, Collection<Object> params, Contributor cb, String lang) throws Exception {
		...
		try {
			...
			List<ErrorReport> list = (List<ErrorReport>) DatabaseHelper.execute("from ErrorReport where url like '%" + url + "' order by id");
	...
	public static StringBuffer getRecordInfo(HttpServletRequest request, String type, int id, String lang) throws Exception {
		...
		if (type.equals(Athlete.alias)) {
			...
			// Other years
			l = DatabaseHelper.execute("select id, year.label, comment, date2 from Result rs where sport.id=" + r.getSport().getId() + " and championship.id=" + r.getChampionship().getId() + " and event.id=" + r.getEvent().getId() + (r.getSubevent() != null ? " and subevent.id=" + r.getSubevent().getId() : "") + (r.getSubevent2() != null ? " and subevent2.id=" + r.getSubevent2().getId() : "") + " order by year.id, rs.id");
			if (l != null && l.size() > 1) {
				String path = r.getSport().getLabel() + "/" + r.getChampionship().getLabel() + "/" + r.getEvent().getLabel() + (r.getSubevent() != null ? "/" + r.getSubevent().getLabel() : "") + (r.getSubevent2() != null ? "/" + r.getSubevent2().getLabel() : "");
				...
				html.append("<tr><td>").append(sbOtherYears.toString()).append("</td></tr></tbody></table>");
			}
			// Rounds
	...
	public static StringBuffer getRecordRef(HttpServletRequest request, ArrayList<Object> params, Collection<Object> coll, boolean isExport, Contributor m, String lang) throws Exception {
		...
		for (Object obj : coll) {
			...
			if (en.equals(Athlete.alias)) {
				...
				c3 = HtmlUtils.writeLink(Year.alias, item.getIdRel1(), item.getLabelRel1(), null);
				c5 = "<span class='details'>" + HtmlUtils.writeLink(Result.alias, idResult, "<img alt='details' title='" +  ResourceUtils.getText("details", lang) + "' src='/img/render/details.png'/>", path) + "</span><table style='margin-right:20px;'><tr>" + p1 + "<td>" + (StringUtils.notEmpty(tEntity[0]) ? tEntity[0] : "-") + "</td>" + (isScore ? "<td>" + StringUtils.formatResult(item.getTxt1(), lang) + "</td>" : "") + (StringUtils.notEmpty(tEntity[1]) ? p2 + "<td>" + tEntity[1] + "</td>" : "") + (StringUtils.notEmpty(tEntity[2]) && !isScore ? p3 + "<td>" + tEntity[2] + "</td>" : "") + "</tr></table>";
				...
				c3 = HtmlUtils.writeLink(Year.alias, item.getIdRel1(), item.getLabelRel1(), null);
				c4 = item.getLabel();
				c5 = "<table><tr><td>" + tEntity[0] + "</td><td>" + StringUtils.formatResult(item.getTxt1(), lang) + "</td>" + (StringUtils.notEmpty(tEntity[1]) ? "<td>" + tEntity[1] + "</td>" : "") + (StringUtils.notEmpty(tEntity[2]) ? "<td>" + tEntity[2] + "</td>" : "") + "</tr></table>";
				c6 = (StringUtils.notEmpty(item.getDate2()) ? HtmlUtils.writeDateLink(null, item.getDate2(), StringUtils.toTextDate(item.getDate2(), lang, "d MMM yyyy")) : "");
				c6_id = (StringUtils.notEmpty(item.getDate2()) ? "dt-" + new SimpleDateFormat("yyyyMMdd").format(item.getDate2()) + "-" + item.getIdItem() : null);
			}
			else if (en.equals(Team.alias)) {
	...
	public static StringBuffer convertResults(HttpServletRequest request, Collection<Object> coll, Championship cp, Event ev, Contributor m, String lang) throws Exception {
	...
	public static StringBuffer convertCalendarResults(HttpServletRequest request, Collection<Object> coll, Contributor m, String lang) throws Exception {
		if (coll == null || coll.isEmpty())
			return new StringBuffer(HtmlUtils.writeNoResult(lang));
		Timestamp lastUpdate = null;
