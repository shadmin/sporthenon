...
public class JExtLinksPanel extends JSplitPane implements ActionListener, ListSelectionListener {
	...
	private void loadLinks() {
		try {
			...
			StringBuffer hql = new StringBuffer("from ExternalLink where entity='" + alias + "'");
			if (tIds.length > 1)
				hql.append(" and idItem between " + tIds[0] + " and " + tIds[1]);				
			if (StringUtils.notEmpty(text)) {
				hql.append(" and (0=1" + (text.matches("\\d+") ? " or idItem=" + text : ""));
				hql.append(" or lower(url) like '%" + text.toLowerCase() + "%'");
				hql.append(")");
			}
