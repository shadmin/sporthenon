/*
 requete de reprise prod :
 
 select * from athlete
where (last_name, first_name, id_sport) in (
	select last_name, first_name, id_sport--, count(*)
	from athlete
	group by last_name, first_name, id_sport
	having count(*) > 1
) and link is null
order by last_name, first_name;
 */

CREATE OR REPLACE FUNCTION public.athlete_link()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
DECLARE
	_id_link integer;
BEGIN
	IF pg_trigger_depth() = 1 AND (NEW.link IS NULL OR NEW.link <> 0) THEN
		SELECT MIN(id)
		INTO _id_link
		FROM athlete
		WHERE (last_name = NEW.last_name OR REGEXP_REPLACE(last_name, '\-.+', '') = NEW.last_name || '-%' OR REGEXP_REPLACE(NEW.last_name, '\-.+', '') = last_name)
			AND first_name = NEW.first_name
			AND id_sport = NEW.id_sport
			AND id <> NEW.id;

		IF _id_link IS NOT NULL THEN
			UPDATE athlete
			SET link = _id_link
			WHERE id = NEW.id;
...
CREATE TRIGGER trigger_athlete_link
    AFTER INSERT OR UPDATE 
    ON public.athlete
    FOR EACH ROW
    EXECUTE PROCEDURE public.athlete_link();
