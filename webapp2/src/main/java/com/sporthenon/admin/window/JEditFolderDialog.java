...
public class JEditFolderDialog extends JDialog implements ActionListener {
	...
	private static final Logger log = Logger.getLogger(JEditFolderDialog.class.getName());
	
	private JList<PicklistItem> jList1 = null;
	private JList<PicklistItem> jList2 = null;
...
//		jList1.addListSelectionListener(this);
	...
	public void open(JResultsPanel parent, List<PicklistItem> list) {
		this.parent = parent;
		DefaultListModel<PicklistItem> model1 = (DefaultListModel<PicklistItem>)jList1.getModel();
		DefaultListModel<PicklistItem> model2 = (DefaultListModel<PicklistItem>)jList2.getModel();
		List<PicklistItem> list_ = new ArrayList<PicklistItem>();
		model1.clear();
		for (PicklistItem plb : parent.getTreeItems()) {
			boolean isSelected = false;
			for (PicklistItem plb_ : list) {
		...
		model2 = (DefaultListModel<PicklistItem>)jList2.getModel();
		model2.clear();
		for (PicklistItem plb : list_)
	...
	private void moveItems(JList<PicklistItem> l1, JList<PicklistItem> l2) {
		DefaultListModel<PicklistItem> model1 = (DefaultListModel<PicklistItem>)l1.getModel();
		DefaultListModel<PicklistItem> model2 = (DefaultListModel<PicklistItem>)l2.getModel();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.matches("\\D\\D\\-(add|find)")) {
			...
			try {
				...
				DefaultListModel<PicklistItem> model = (DefaultListModel<PicklistItem>)jList2.getModel();
				for (int i = 0 ; i < model.getSize() ; i++) {
					PicklistItem plb = (PicklistItem) model.getElementAt(i);
