	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (hParams.containsKey("auth")) {
				...
				String sql = "SELECT * FROM FROM _contributor WHERE LOWER(login) = ? and password = ?";
				List<Contributor> contributors = (List<Contributor>) DatabaseManager.executeSelect(sql, Arrays.asList(new Object[]{login.toLowerCase(), StringUtils.toMD5(password)}), Contributor.class);
				...
				List<Contributor> l = (List<Contributor>) DatabaseManager.executeSelect("SELECT * FROM FROM _contributor WHERE login = '" + hParams.get("rlogin") + "'", Contributor.class);
				if (l != null && l.size() > 0)
