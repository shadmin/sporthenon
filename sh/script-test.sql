...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "GET_US_CHAMPIONSHIPS"(integer, text, character varying) OWNER TO postgres;



ALTER TABLE "CHAMPIONSHIP" DROP COLUMN inactive;
ALTER TABLE "EVENT" DROP COLUMN inactive;

ALTER TABLE "RESULT" ADD id_subevent2 INTEGER;
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "GET_RESULTS"(integer, integer, integer, integer, integer, text, character varying) OWNER TO postgres;
DROP FUNCTION "GET_RESULTS"(integer, integer, integer, integer, text, character varying);




CREATE OR REPLACE FUNCTION "TREE_RESULTS"(_lang character varying)
  RETURNS SETOF "~TREE_ITEM" AS
$BODY$
declare
	_item "~TREE_ITEM"%rowtype;
...
begin
	...
	_current_se_id := 0;
	OPEN _c FOR EXECUTE
	'SELECT DISTINCT SP.id, SP.label' || _lang || ', CP.id, CP.label' || _lang || ', EV.id, EV.label' || _lang || ', SE.id, SE.label' || _lang || ', SE2.id, SE2.label' || _lang || ', II.id_sport, CP.index, EV.index, SE.index, SE2.index
	    FROM "RESULT" RS LEFT JOIN "SPORT" SP ON RS.id_sport = SP.id
	    LEFT JOIN "CHAMPIONSHIP" CP ON RS.id_championship = CP.id
	    LEFT JOIN "EVENT" EV ON RS.id_event = EV.id
	    LEFT JOIN "EVENT" SE ON RS.id_subevent = SE.id
	    ...
	    ORDER BY SP.label' || _lang || ', CP.index, EV.index, SE.index, CP.label' || _lang || ', EV.label' || _lang || ', SE.label' || _lang;
	LOOP
		FETCH _c INTO _sp_id, _sp_label, _cp_id, _cp_label, _ev_id, _ev_label, _se_id, _se_label, _se2_id, _se2_label, _ii_sport;
		EXIT WHEN NOT FOUND;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "TREE_RESULTS"(character varying) OWNER TO postgres;
...
CREATE SEQUENCE "~SQ_INACTIVE_ITEM"
  ...
  START 1
  CACHE 1;
ALTER TABLE "~SQ_INACTIVE_ITEM" OWNER TO postgres;
...
ALTER TABLE "~REF_ITEM" ADD label_rel12 character varying(50);
...
begin
	...
	IF (_entity ~ 'CN|PR|TM|CP|EV|CT|SP|CX|OL|YR' AND (_entity_ref = 'RS' OR _entity_ref = '')) THEN
		...
		OPEN _c FOR EXECUTE _query;
		LOOP
			FETCH _c INTO _item.id_item, _item.id_rel1, _item.label_rel1, _item.id_rel2, _item.label_rel2, _item.id_rel3, _item.label_rel3, _item.id_rel4, _item.label_rel4, _item.id_rel5, _item.label_rel5, _item.id_rel12, _item.label_rel12, _id1, _id2, _id3, _id4, _id5, _id6, _id7, _id8, _id9, _id10, _type1, _type2, _type3;
			EXIT WHEN NOT FOUND;
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "GET_DRAW"(integer, character varying) OWNER TO postgres;
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "WIN_RECORDS"(text, character varying) OWNER TO postgres;
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "~LAST_UPDATES"(integer, character varying) OWNER TO postgres;
...
begin
	...
	'SELECT 
		...
		ST2.label' || _lang || ' AS st2_label, CN1.id AS cn1_id, CN1.code AS cn1_code_, CN1.label' || _lang || ' AS cn1_label_, CN2.id AS cn2_id, CN2.code AS cn2_code_, CN2.label' || _lang || ' AS cn2_label_,
		RS.id_rank1 AS rs_rank1, RS.id_rank2 AS rs_rank2, RS.id_rank3 AS rs_rank3, RS.id_rank4 AS rs_rank4, RS.id_rank5 AS rs_rank5,
		RS.result1 AS rs_result1, RS.result2 AS rs_result2, RS.result3 AS rs_result3, TP1.number AS tp1_number, TP2.number AS tp2_number, TP3.number AS tp3_number, OL.id AS ol_id, OL.type AS ol_type, OL.date1 AS ol_date1, OL.date2 AS ol_date2, CT3.label' || _lang || ' AS ol_city, RS.comment as rs_comment, RS.exa as rs_exa'
		|| _columns || '
	FROM
		"RESULT" RS
		LEFT JOIN "EVENT" EV ON RS.id_event = EV.id
...
DROP FUNCTION "GET_OLYMPIC_MEDALS"(text, integer, text, text, character varying);


DROP FUNCTION "TREE_RESULTS"(character varying);


CREATE OR REPLACE FUNCTION "TREE_RESULTS"(_filter varchar, _lang character varying)
  RETURNS SETOF "~TREE_ITEM" AS
$BODY$
declare
	_item "~TREE_ITEM"%rowtype;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "TREE_RESULTS"(character varying, character varying) OWNER TO postgres;
...
ALTER TABLE "~REF_ITEM" ADD id_rel18 integer;
ALTER TABLE "~REF_ITEM" ADD label_rel18 character varying(50);
...
CREATE OR REPLACE FUNCTION "SEARCH"(_pattern character varying, _scope character varying, _ref smallint, _lang character varying)
  RETURNS SETOF "~REF_ITEM" AS
$BODY$
declare
	_item "~REF_ITEM"%rowtype;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "SEARCH"(character varying, character varying, smallint, character varying) OWNER TO inachos;
