...
declare
	...
	_limit varchar(3);
begin
	_limit := CASE _entity_ref WHEN '' THEN '50' ELSE 'ALL' END;
...
declare
	...
	_limit varchar(3);
begin
	_limit := CASE _entity_ref WHEN '' THEN '20' ELSE 'ALL' END;
...
$BODY$
  ...
  DROP FUNCTION "ENTITY_REF"(character varying, integer, character varying, character varying);
CREATE OR REPLACE FUNCTION "ENTITY_REF"(_entity character varying, _id integer, _entity_ref character varying, _limit varchar, _offset varchar, _lang character varying)
  RETURNS SETOF "~REF_ITEM" AS
$BODY$
declare
	_item "~REF_ITEM"%rowtype;
...
begin
	...
	IF (_entity ~ 'CN|SP|TM' AND (_entity_ref = 'PR' OR _entity_ref = '')) THEN
		...
		ELSIF _entity = 'TM' THEN
			_query := _query || ' WHERE PR.id_team = ' || _id;
		END IF;
		_query := _query || ' ORDER BY PR.last_name, PR.first_name LIMIT ' || _limit || ' OFFSET ' || _offset;
		OPEN _c FOR EXECUTE _query;
		LOOP
			FETCH _c INTO _item.id_item, _item.label, _item.id_rel1, _item.label_rel1, _item.id_rel2, _item.label_rel2, _item.id_rel3, _item.label_rel3;
			EXIT WHEN NOT FOUND;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "ENTITY_REF"(character varying, integer, character varying, character varying, character varying, character varying) OWNER TO postgres;



DROP FUNCTION "ENTITY_REF"(character varying, integer, character varying, character varying, character varying, character varying);
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "ENTITY_REF"(character varying, integer, character varying, character varying, integer, character varying) OWNER TO postgres;
