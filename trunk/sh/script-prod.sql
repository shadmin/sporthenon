...
DROP FUNCTION "GET_HALL_OF_FAME"(integer, text);

CREATE OR REPLACE FUNCTION "GET_HALL_OF_FAME"(_id_league integer, _years text, _position varchar)
  RETURNS refcursor AS
$BODY$
declare
    _c refcursor;
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION "GET_HALL_OF_FAME"(integer, text, varchar) OWNER TO postgres;

UPDATE "HALL_OF_FAME" SET position = 'E' where position='End';

DROP FUNCTION "GET_US_RECORDS"(integer, text, text, text);
